        ...
        {(Object.keys(PLATFORM_CONFIG) as PlatformId[]).map((platformId) => {
          ...
          return (
            <button
              ...
              className={`
                relative flex items-center gap-3 p-4 rounded-md border text-left transition-all duration-200 group outline-none focus:ring-2 focus:ring-black dark:focus:ring-white focus:ring-offset-2 dark:focus:ring-offset-black
                ${isSelected 
                  ? 'bg-gray-50 border-black dark:bg-gray-900 dark:border-white' 
              ...
              `}
            >
              <div className={`p-2 rounded-full ${isSelected ? 'bg-white dark:bg-black' : 'bg-gray-50 dark:bg-gray-900'}`}>
                <Icon size={20} strokeWidth={1.5} className={isSelected ? 'text-black dark:text-white' : 'text-gray-500'} />
              </div>
              <div className="flex-1">
                <span className={`block text-sm font-medium ${isSelected ? 'text-black dark:text-white' : 'text-gray-600 dark:text-gray-300'}`}>
                  {config.label}
                </span>
              ...
              {isSelected && (
                <div className="absolute top-2 right-2 text-black dark:text-white">
                  <CheckCircle2 size={14} fill="currentColor" className="text-black dark:text-white bg-white dark:bg-black rounded-full" />
                </div>
              )}
            </button>
          );
